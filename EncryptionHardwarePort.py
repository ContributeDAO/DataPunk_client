...
class EncryptionHardwarePort:
    ...
    def sign_hash(self,key_id: int, hash_value):
        """
        签名 (SH)。
        """
        if(key_id > 127):
        ...
        #hash_value_bytes = bytes.fromhex(hash_value)  # 确保hash_value是bytes类型
        #hash_length = len(hash_value).to_bytes(1, 'big', signed=True)
        return self.send_command(command_prefix, key_id_bytes +  hash_value)
